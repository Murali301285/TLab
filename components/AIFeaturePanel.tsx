...
import {
    ...
    MessageSquare, Layers, Presentation, Lightbulb, Play, Pause, Square, GalleryVerticalEnd, LayoutGrid, Network, Workflow
} from 'lucide-react';
...
interface AIFeaturePanelProps {
    ...
    activeTab: 'summary' | 'mindmap' | 'quiz' | 'flashcards' | 'explain' | 'podcast' | 'simplify' | 'visualize' | 'scrollytelling';
...
export default function AIFeaturePanel({
    ...
    }, [activeTab, isOpen, quizConfig, quiz.length]); // Added quiz.length to trigger regen on reset
    ...
    const handleAIFeature = async (feature: string, overrideText?: string, analogyOverride?: string, quizConfigOverride?: number) => {
        ...
        try {
            ...
            const parseJSON = (str: any) => {
                ...
                try {
                    ...
                    if (!cleanStr.startsWith('{') && !cleanStr.startsWith('[')) {
                        // Detect legacy mermaid or invalid strings
                        console.warn("Invalid JSON string detected", cleanStr.substring(0, 50));
                        return null;
                    }
                    return JSON.parse(cleanStr);
                } catch (e) {
                    console.error("JSON Parse Error", e);
                    // Return null so the UI doesn't crash
                    return null;
                }
            };

            if (feature === 'summary') setSummary(content);
            else if (feature === 'explain') setExplanation(content);
            else if (feature === 'simplify') setSimplificationData(content);
            else if (feature === 'quiz') {
                setQuiz(parseJSON(content) || []);
            ...
            else if (feature === 'mindmap') setMindMapData(parseJSON(content));
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 border-l border-slate-200 w-full animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shrink-0 h-16">
                ...
                <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-lg">
                    <LayoutGrid className="h-4 w-4" /> {/* Close Icon Replacement */}
                </button>
            </div>
            ...
            <div className="flex overflow-x-auto bg-white border-b border-slate-200 shrink-0 scrollbar-hide">
                {[
                    { id: 'summary', icon: BrainCircuit, label: 'Summary', color: 'cyan' },
                    ...
                    <div className="h-full">
                        ...
                        {activeTab === 'summary' && (
                            <div className="prose prose-sm prose-slate max-w-none">
                                {summary ? (
                                    ...
                                    <div className="text-center text-slate-400 mt-20">No summary generated yet.</div>
                                )}
                            </div>
                        )}
                        ...
                        {(activeTab === 'simplify' || activeTab === 'explain') && (
                            <div className="space-y-4">
                                ...
                                <div className="prose prose-sm prose-amber bg-amber-50/50 p-4 rounded-xl border border-amber-100">
                                    ...
                                    {visualizeData ? (
                                        <>
                                            ...
                                            {visualLayout === 'mindmap' && (
                                                mindMapData ? <MindMapRenderer data={mindMapData} /> : (isAiLoading ? <div className="text-center text-slate-400 mt-20">Generating detailed mind map...</div> : (visualizeData?.graph ? <MindMapRenderer data={visualizeData} /> : <div className="text-center text-slate-400 mt-20">Click to generate mind map.</div>))
                                            )}
                                        </>
                                    ) : (
